...
def test_read_problems(mock_read_problems, fake_session):
    """Test that read_problems returns a list of problems."""
    mock_list = [ProblemGet(problem_id=1, title="prob", description="descript")]
    mock_read_problems.return_value = mock_list

    result = actions.read_problems(fake_session, offset=0, limit=10)

    mock_read_problems.assert_called_once_with(fake_session, 0, 10)
    assert result == mock_list


@patch("db.api.modules.actions.ops.get_submissions")
def test_read_submissions(mock_get_submissions, fake_session):
    ...
    result = actions.read_submissions(fake_session, offset=0, limit=10)

    mock_get_submissions.assert_called_once_with(fake_session, 0, 10)
